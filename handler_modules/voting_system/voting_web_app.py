...
from handler_modules.voting_system.exceptions import (
...
class Vote(Handler):
    ...
    def answer(self, result):
        ...
        if isinstance(member, ChatMemberLeft):
            self.message.reply_text("Вы не являетесь членом нужного чата!")
            return
...
class VotingWebApp(Handler):
    def __call__(self, update: Update, context: ContextTypes.context):
        ...
        if isinstance(member, ChatMemberLeft):
            update.effective_message.reply_text("Вы не являетесь членом нужного чата!")
            return
        ...
        if (not vs) or vs.is_finished:
            ...
            if stage != vs.stage:
                reply_text = (
                    "Вы пытаетесь подать голоса из предыдущей стадии голосования"
                )
                ...
                try:
                    ...
                    reply_text = (
                        f"<b>Вы </b>({update.effective_user.name}#{update.effective_user.id})"
